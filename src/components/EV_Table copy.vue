                            ...
                            <div>
                                ...
                                <template v-else>
                                    ...
                                    <span v-if="col.sortable">
                                        <i @click="handleSort(col.key, 'asc')"
                                            >↑</i
                                        >
                                        <i @click="handleSort(col.key, 'desc')"
                                            >↓</i
                                        >
                                    </span>
                                </template>
                            ...
                            <td v-for="col in columns" :key="col.key">
                                <div>
                                    ...
                                    <template v-else-if="col.type === 'expand'">
                                        <span @click="toggleExpand(row.id)"
                                            >></span
                                        >
                                    </template>
                                    <template v-else>
    ...
    methods: {
        ...
        toggleSelect(e, row) {
            ...
            if (checked) {
                ...
                let idx = this.selectedRows.findIndex(
                    (item) => item.id === row.id
                )
        ...
        formateStatus(row) {
            return (
                this.selectedRows.findIndex((item) => item.id === row.id) >= 0
            )
        },
